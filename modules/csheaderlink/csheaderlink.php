...
class CsHeaderlink extends Module
{
	public function __construct()
	{
		$this->name = 'csheaderlink';
		$this->tab = 'Orther Module';
		$this->version = 0.1;
		...
		$this->displayName = $this->l('CS Header links block');
		$this->description = $this->l('Adds a block that displays information about the customer.');
	}

	public function install()
	{
		return (parent::install() AND $this->registerHook('displaytoptop') AND $this->registerHook('header'));
	}
	...
	*/
	public function hookDisplayTopTop($params)
	{
		...
		return $this->display(__FILE__, 'csheaderlink.tpl');
	}
	...
	public function hookHeader($params)
	{
		...
		$this->context->controller->addCSS(($this->_path).'csheaderlink.css', 'all');
	}
}
