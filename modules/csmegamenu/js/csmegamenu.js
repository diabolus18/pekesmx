...
$(document).ready(function(){
	$('.select_type_option').attr('onchange', 'return displayFormOption();');
	if($('#type_option').hasClass('select_type_option'))
		displayFormOption();
});

function displayFormOption()
{
	option_type = $('.select_type_option option:selected').val();
	$('.cs_hide_option .form-group').each(function( index ) {
			if(index >= 0 && index <=2)
				$(this).show();
			else
				$(this).hide();
	...
	{
	case "0":
		$('.cs_hide_option').children().children().children('.form-group').each(function( index ) {
			if(index >= 3 && index <=8)
				$(this).show();
		});
	  break;
