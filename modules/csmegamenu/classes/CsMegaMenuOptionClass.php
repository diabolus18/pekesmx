...
class CsMegaMenuOptionClass extends ObjectModel
{
	...
	public static $definition = array(
		...
		'fields' => array(
			//Shop fields
			'id_csmegamenu' =>		array('type' => self::TYPE_INT,'shop' => true, 'validate' => 'isunsignedInt'),
	...
	{
		$errors = '';//var_dump($_POST['content_option'][$_POST['type_option']]['opt_id_parent_cat']);die;
		/*check validate*/
		...
		if ($_POST['type_option'] == 2) //static
		{
			...
			if(!isset($_POST['opt_content_static_'.$defaultLanguage.'']) || $_POST['opt_content_static_'.$defaultLanguage.''] == '')
	...
	public function copyFromPostOption()
	{
		...
		if ($_POST['type_option'] == 2)
		{
			foreach ($languages AS $language)
			{
				$_POST['content_option'][2]['opt_content_static'][(int)($language['id_lang'])] = $_POST['opt_content_static_'.$language['id_lang'].''];
			}
			
		}
		else if ($_POST['type_option'] == 0)
		{			
			$_POST['content_option'][0]['opt_id_parent_cat'] = $_POST['content_option'][$_POST['type_option']]['opt_id_parent_cat'];
		}
	...
	public function updatePositionOptionMax($id_shop)
	{
		$result = (Db::getInstance()->Execute('
					...
					SET `position_option`='.$this->getNextPositionOption($id_shop,$this->id_csmegamenu).'
					WHERE `id_shop` = '.$id_shop.' AND `id_option`='.(int)($this->id)));
		return $result;
	}
	
	public static function getNextPositionOption($id_shop,$id_csmegamenu)
	{
		$max = Db::getInstance()->getValue('SELECT MAX(position_option)+1 FROM `'._DB_PREFIX_.'csmegamenu_option_shop` WHERE id_csmegamenu = '.$id_csmegamenu.($id_shop ? ' AND `id_shop` = '.$id_shop : '1').' ' );
		return ($max ? $max : 0);
	}
		...
		if (!isset($position))
			...
			$result = (Db::getInstance()->Execute('
				...
				'.($way
					...
					: '< '.(int)($option_move['position_option']).' AND `position_option` >= '.(int)($position)).'
					AND `id_csmegamenu` = '.$this->id_csmegamenu.'
					')
