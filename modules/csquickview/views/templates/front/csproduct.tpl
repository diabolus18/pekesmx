...
<div id="primary_block" class="clearfix">
			...
			<ul id="thumbs_list_frame">
				{if isset($images)}
					...
					<li id="thumbnail_{$image.id_image}">
						<a href="{$link->getImageLink($product->link_rewrite, $imageIds, thickbox_default)}" rel="other-views" class="{if $smarty.foreach.thumbnails.first}shown{/if}" title="{$image.legend|htmlspecialchars}" onclick="return false">
	...
	<div id="pb-left-column">
		<h3><a href="{$link->getProductLink($product)}" target="_parent" title="">{$product->name|escape:'htmlall':'UTF-8'}</a></h3>
		...
		<div class="content_prices clearfix">
			...
			<div id="reduction_percent" {if !$product->specificPrice OR $product->specificPrice.reduction_type != 'percentage'} style="display:none;"{/if}><span id="reduction_percent_display">{if $product->specificPrice AND $product->specificPrice.reduction_type == 'percentage'}-{$product->specificPrice.reduction*100}%{/if}</span></p>
			...
			<div class="price">
				...
				{/if}
				{if $priceDisplay >= 0 && $priceDisplay <= 2}
					<span id="our_price_display" style="font-size:1.5em">{convertPrice price=$productPrice}</span>
			...
			<!-- quantity wanted -->
				...
				<span class="csbutton csdefault">					
					{l s='Out of stock' mod='csquickview'}
				</span>
			{else}
				<p id="add_to_cart">					
					<input type="submit" name="Submit" value="{l s='Add to cart' mod='csquickview'}" class="csbutton csdefault" onclick="return closeFancy()" />
				</p>
			{/if}
			</div>
				</div>
