...
class SliderClass extends ObjectModel
{
	...
	public static $definition = array(
		...
		'fields' => array(
			// basic fields
			'image' =>	 array('type' => self::TYPE_STRING,'shop' => true, 'validate' => 'isString'),
			//Shop fields
			'animation' => array('type' => self::TYPE_HTML, 'shop' => true, 'validate' => 'isString'),
			'position' =>		array('type' => self::TYPE_INT,'shop' => true, 'validate' => 'isunsignedInt'),
			'display'  => 		array('type' => self::TYPE_BOOL,'shop' => true),
			// Lang fields
			'url' => 	array('type' => self::TYPE_STRING,'lang' => true, 'validate' => 'isString', 'size' => 300)
		)
	);
	...
	public function updateStatus($status)
	{
		...
		$result = (Db::getInstance()->Execute('
			...
			SET `display`='.$status.'
			WHERE `id_shop` = '.$id_shop.' AND `id_csslider`='.(int)($this->id_csslider)));
		return $result;
	}
	...
	public function getFieldShop($field)
	{
		...
		LEFT JOIN '._DB_PREFIX_.'csslider_shop ss ON (s.id_csslider = ss.id_csslider)
		WHERE s.id_csslider = '.$this->id.' AND ss.id_shop = '.$id_shop.'';
	...
	public function updateInforImage($id_shop)
	{
		$slider_update = Db::getInstance()->getRow(
					...
					AND ss.`id_csslider` = '.(int)($this->id));							
		...
		$sql = 'UPDATE `'._DB_PREFIX_.'csslider_shop`
					SET `image` = \''.$new_image.'\'
					WHERE `id_shop` = '.$id_shop.' AND `id_csslider` = '.(int)($this->id);
	...
	public function uploadImage($id_csslider = null)
	{
		$errors = "";
		if (isset($_FILES['image']) AND isset($_FILES['image']['tmp_name']) AND !empty($_FILES['image']['tmp_name']))
		{
			...
			{
				$ext = substr($_FILES['image']['name']['value'], $dot_pos+1);
				$newname = ($id_csslider ? $id_csslider : $this->getNextId());
	...
	public function validateController($htmlentities = true, $copy_post = false)
	{
		...
		if($_POST['image']['type'] == "image")
		{
			...
			if($error = $this->uploadImage($this->id_csslider))
			{
				$errors[] = $error;
				return $errors;
			}
	...
	{
		$max = Db::getInstance()->getValue('SELECT MAX(id_csslider) FROM `'._DB_PREFIX_.'csslider`');
		return ($max ? $max + 1 : 1);
	}
	...
	public function getCaption()
	{
		if (!$result = Db::getInstance()->ExecuteS(
			...
			RIGHT JOIN `'._DB_PREFIX_.'csslider_caption` csc ON (cs.`id_csslider` = csc.`id_csslider`) 
			WHERE cs.`id_csslider` = '.(int)($this->id_csslider).' ORDER BY csc.`layer` ASC'))
			return false;
	 	return $result;
	}
	...
	public function updatePosition($way, $position)
	{
		if (!isset($position))
			...
			$slider_move = Db::getInstance()->getRow(
					'SELECT ss.* FROM `'._DB_PREFIX_.'csslider_shop` ss
					WHERE (ss.id_shop = '.(int)$id_shop.')
					AND ss.`id_csslider` = '.(int)($this->id_csslider));
			$result = (Db::getInstance()->Execute('
				UPDATE `'._DB_PREFIX_.'csslider_shop`
	...
	public static function cleanPositions()
	{
		...
		for ($i = 0; $i < $sizeof; $i++){
				...
				UPDATE `'._DB_PREFIX_.'csslider_shop`
				SET `position` = '.(int)($i).'
				WHERE `id_shop` = '.$id_shop.' AND `id_csslider` = '.(int)($result[$i]['id_csslider']);
				$return &= Db::getInstance()->Execute($sql);
			}
		return $return;
	}
