...
class StaticBlockClass extends ObjectModel
{
	...
	public function getFieldShop($field)
	{
		...
		WHERE b.id_staticblock = '.$this->id.' AND bs.id_shop = '.$id_shop.'';
	...
	public function uniqueIndentifier_block($identifier_block, $id_staticblock = null)
	{
		$result = Db::getInstance()->getRow('
			SELECT `identifier_block`
			FROM `'._DB_PREFIX_.'staticblock` 
			WHERE `identifier_block` LIKE \''.$identifier_block.'\''.($id_staticblock ? ' AND `id_staticblock` != '.$id_staticblock : ''));
	...
	public function updateStatus($status)
	{
		...
		$result = (Db::getInstance()->Execute('
			...
			WHERE `id_shop` = '.$id_shop.' AND `id_staticblock`='.(int)($this->id_staticblock)));
		return $result;
	}
	public function validateController($htmlentities = true, $copy_post = false)
	{
		...
		if ($this->identifier_block and !$this->uniqueIndentifier_block($this->identifier_block, (int)($this->id_staticblock)))
		{
