...
class AdminCSCategory extends ModuleAdminController
{
	function __construct()
	{
		global $cookie;
		$this->className = 'CSPLCategory';
		$this->table = 'cs_blog_category';
		$this->bootstrap = true;
		...
		$this->fieldImageSettings = array(
 			'name' => 'image',
 			'dir' => _PS_MODULE_DIR_.'csblog/media/categories/src'
 		);
		$this->fields_list = array(
			'id_cs_blog_category' => array(
				'title' => $this->l('ID'),
				'align' => 'center',
				'filter_key' => 'a!id_cs_blog_category',
				'width' => 20
			),
			'name' => array(
				'title' => $this->l('Name'), 
	...
	public function init()
	{
		...
		if (Shop::getContext() == Shop::CONTEXT_SHOP)
			$this->_join .= ' LEFT JOIN `'._DB_PREFIX_.'cs_blog_category_shop` sa ON (a.`id_cs_blog_category` = sa.`id_cs_blog_category` AND sa.id_shop = '.(int)$this->context->shop->id.') ';
		else
			$this->_join .= ' LEFT JOIN `'._DB_PREFIX_.'cs_blog_category_shop` sa ON (a.`id_cs_blog_category` = sa.`id_cs_blog_category` AND sa.id_shop = '.$id_shop_default.') ';
		...
		$this->_where .= ' AND a.category_parent = 1';
		if (Tools::isSubmit('viewcs_blog_category'))
			$id_parent = Tools::getValue('id_cs_blog_category');
		else 
			$id_parent = 1;
	...
	public function renderForm()
	{
		...
		//
		$html_image_des = '';
		$id_cs_blog_category = Tools::getValue('id_cs_blog_category');
		if($id_cs_blog_category)
			$html_image_des .= $this->displayImage($id_cs_blog_category);
		...
		$cats = $this->object->getCategories($id_lang,$id_shop,false);
			...
			'input' => array(
				...
				array(
				 ...
				 array(
					...
					'delete_url' => self::$currentIndex.'&id_cs_blog_category='.$id_cs_blog_category.'&updatecs_blog_category&token='.$this->token.'&deleteimageCategory',
	...
	public function postProcess()
	{
		...
		{
			$id = Tools::getValue('id_cs_blog_category');
	...
	public function displayImage($id_cs_blog_category)
	{
		
		if ($id_cs_blog_category) {
			$save_path = _PS_MODULE_DIR_.'csblog/media/categories/src/'.$id_cs_blog_category;
			$url_path = '../modules/csblog/media/categories/src/'.$id_cs_blog_category;
	...
	public function getList($id_lang, $order_by = null, $order_way = null, $start = 0, $limit = null, $id_lang_shop = false)
	{
		...
		{
			$item = &$this->_list[$i];
			$category_tree = CSPLCategory::getChildren((int)$item['id_cs_blog_category'], $this->context->language->id, true,$this->context->shop->id);
			if (!count($category_tree))
				$this->addRowActionSkipList('view', array($item['id_cs_blog_category']));
		}
	...
	{
		if($id){
			$save_path = _PS_MODULE_DIR_.'csblog/media/categories/src/'.$id;
			$url_path_general = _PS_MODULE_DIR_.'csblog/media/categories/cache/'.$id;
			$src_path = $save_path.'.'.$this->imageType;
			foreach (Shop::getContextListShopID() as $id_shop)
			{
				
				$des_path = $url_path_general.'_'.$id_shop.'_category.'.$this->imageType;
				$this->imageResizeThumb($src_path,$des_path,Configuration::get('CS_IMG_CATEGORY_SIZE',null,null,$id_shop),Configuration::get('CS_IMG_CATEGORY_H_SIZE',null,null,$id_shop));
			}
