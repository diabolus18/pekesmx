<?php
class CSPLCategory extends ObjectModel
{
	...
	public static $definition = array(
		'table' => 'cs_blog_category',
		'primary' => 'id_cs_blog_category',
		'multilang' => true,
		'multilang_shop' => true,
		'fields' => array(
			//basic fields
	...
	public	function __construct($cs_blog_category = null, $id_lang = null, $id_shop = null, Context $context = null)
	{
		...
		Shop::addTableAssociation('cs_blog_category', array('type' => 'shop'));
		Shop::addTableAssociation('cs_blog_category_lang', array('type' => 'fk_shop'));
		
	}
	public function getFieldShop($field)
	{
		$id_shop = (int)Context::getContext()->shop->id;
		$sql = 'SELECT ss.'.$field.' FROM '._DB_PREFIX_.'cs_blog_category s
		LEFT JOIN '._DB_PREFIX_.'cs_blog_category_shop ss ON (s.id_cs_blog_category = ss.id_cs_blog_category)
		WHERE s.id_cs_blog_category = '.$this->id.' AND ss.id_shop = '.$id_shop.'';
			...
			if (Shop::getContext() != Shop::CONTEXT_SHOP)
				foreach (Shop::getContextListShopID() as $id_shop)
					$return &= Db::getInstance()->execute('
						INSERT INTO `'._DB_PREFIX_.'cs_blog_category_shop` (`id_cs_blog_category`, `id_shop`, `position`) VALUES
	...
	public static function getLastPosition($id_category_parent, $id_shop)
	{
		...
		FROM `'._DB_PREFIX_.'cs_blog_category` c
		LEFT JOIN `'._DB_PREFIX_.'cs_blog_category_shop` cs
			ON (c.`id_cs_blog_category` = cs.`id_cs_blog_category` AND cs.`id_shop` = '.(int)$id_shop.')
		WHERE c.`category_parent` = '.(int)$id_category_parent) + 1);
	}
	public function calcLevelDepth()
	{
		...
		$parent_category = new CSPLCategory((int)$this->category_parent);
	...
	public function add($autodate = true, $null_values = false)
	{
		...
		if (Tools::isSubmit('checkBoxShopAsso_cs_blog_category'))
			foreach (Tools::getValue('checkBoxShopAsso_cs_blog_category') as $id_shop => $value)
			{
				$position = CSPLCategory::getLastPosition((int)$this->category_parent, $id_shop);
				$this->addPosition($position, $id_shop);
			}
		else
			foreach (Shop::getShops(true) as $shop)
			{
				$position = CSPLCategory::getLastPosition((int)$this->category_parent, $shop['id_shop']);
				if (!$position)
					$position = 1;
	...
	{
		$sql = 'SELECT * FROM '._DB_PREFIX_.'cs_blog_category_shop
				WHERE id_cs_blog_category='.$this->id.'';
	...
	public function deleteSubCatForPlCategory($id)
	{
		...
		foreach($subs as $sub)
		{
			$cat = new CSPLCategory($sub['id_cs_blog_category']);
			$cat->delete();
		}
	...
	public function getSubPlCategory($id_parrent)
	{
		...
		$sql = 'SELECT DISTINCT a.id_cs_blog_category FROM '._DB_PREFIX_.'cs_blog_category a
				LEFT JOIN '._DB_PREFIX_.'cs_blog_category_shop c
				ON (a.id_cs_blog_category = c.id_cs_blog_category)
	...
	{
		$rows = Db::getInstance()->ExecuteS("SELECT id_cs_blog_category FROM "._DB_PREFIX_."cs_blog_category ORDER BY id_cs_blog_category DESC LIMIT 0, 1");
		$row = $rows[0];
		return $row['id_cs_blog_category'];
	}
	...
	public static function getChildren($id_parent, $id_lang, $active = true, $id_shop = false)
	{
		...
		$query = 'SELECT c.`id_cs_blog_category`, cl.`name`, cl.`link_rewrite`, cs.`id_shop`
		FROM `'._DB_PREFIX_.'cs_blog_category` c
		LEFT JOIN `'._DB_PREFIX_.'cs_blog_category_lang` cl ON (c.`id_cs_blog_category` = cl.`id_cs_blog_category`)
		LEFT JOIN `'._DB_PREFIX_.'cs_blog_category_shop` cs ON (c.`id_cs_blog_category` = cs.`id_cs_blog_category`)
		WHERE cl.`id_lang` = '.(int)$id_lang.' AND cs.`id_shop` = '.$id_shop.'
		AND c.`category_parent` = '.(int)$id_parent.'
		'.($active ? 'AND `active` = 1' : '').'
		GROUP BY c.`id_cs_blog_category`
	...
	{
		$sql = 'SELECT bcl.*, bc.*, bcs.position FROM '._DB_PREFIX_.'cs_blog_category bc
				LEFT JOIN '._DB_PREFIX_.'cs_blog_category_lang bcl
				ON (bc.id_cs_blog_category = bcl.id_cs_blog_category AND bcl.id_shop = '.$id_shop.')
				LEFT JOIN '._DB_PREFIX_.'cs_blog_category_shop bcs
				ON (bc.id_cs_blog_category = bcs.id_cs_blog_category)
				WHERE bc.active=1 '.($no_home ? 'AND bc.id_cs_blog_category != 1 ' : '').' AND bcs.id_shop = '.($id_shop ? $id_shop : Configuration::get('PS_SHOP_DEFAULT')).' AND bcl.id_lang='.($id_lang ? $id_lang : Configuration::get('PS_LANG_DEFAULT'));
	...
	{
		$sql = 'SELECT a.id_cs_blog_category FROM '._DB_PREFIX_.'cs_blog_category a
				LEFT JOIN '._DB_PREFIX_.'cs_blog_category_lang b
				ON (a.id_cs_blog_category = b.id_cs_blog_category '.( $id_shop ? 'AND b.id_shop = '.$id_shop : '' ).')
				LEFT JOIN '._DB_PREFIX_.'cs_blog_category_shop c
				ON (a.id_cs_blog_category = c.id_cs_blog_category)
	...
	public function getParentsCategories($id_category , $id_lang = null, $id_shop = null)
	{
		...
		foreach($cats as $row)
		{
			...
			{
				if($row2['id_cs_blog_category'] == $id_parent && $id_parent != 1)
				{
					$resultParents[$row2['id_cs_blog_category']] = $row2;
					$i = $row2['id_cs_blog_category'];
					break;
				}
			}
		}
	...
	public function getParentsCategoriesComplete($cats=array(), $id_lang = null, $id_shop = null)
	{
		...
		if($string != '')
		{
			$sql = 'SELECT a.id_cs_blog_category, b.name, b.link_rewrite FROM '._DB_PREFIX_.'cs_blog_category a
				LEFT JOIN '._DB_PREFIX_.'cs_blog_category_lang b
				ON (a.id_cs_blog_category = b.id_cs_blog_category '.( $id_shop ? 'AND b.id_shop = '.$id_shop : '' ).')
				LEFT JOIN '._DB_PREFIX_.'cs_blog_category_shop c
				ON (a.id_cs_blog_category = c.id_cs_blog_category)
				WHERE 1 AND a.id_cs_blog_category IN ('.$string.') '.( $id_shop ? 'AND c.id_shop = '.$id_shop : '' ).( $id_lang ? ' AND b.id_lang = '.$id_lang : '' );
	...
	public static function getCategoriesCheckbox($id_lang = false, $id_shop = true,$no_home = false)
	{
		...
		foreach ($result as $row)
			$categories[$row['category_parent']][$row['id_cs_blog_category']]['infos'] = $row;
		return $categories;
	}
	...
	public static function getChildrenWithNbSelectedSubCat($id_parent, $selected_cat, $id_lang, Shop $shop = null, $use_shop_context = true)
	{
		...
		$sql = 'SELECT c.`id_cs_blog_category` as id_category, c.`level_depth`, cl.`name`, 
		 IF((
						SELECT COUNT(*)
						FROM `'._DB_PREFIX_.'cs_blog_category` c2
						WHERE c2.`category_parent` = c.`id_cs_blog_category`
					) > 0, 1, 0) AS has_children
		
		, '.($selected_cat ? '(
						SELECT count(c3.`id_cs_blog_category`)
						FROM `'._DB_PREFIX_.'cs_blog_category` c3
						WHERE  c3.`category_parent` = c.id_cs_blog_category AND c3.`id_cs_blog_category` IN ('.$selected_cat.')
					)' : '0').' AS nbSelectedSubCat
		
		FROM `'._DB_PREFIX_.'cs_blog_category` c 
		LEFT JOIN `'._DB_PREFIX_.'cs_blog_category_lang` cl ON (c.`id_cs_blog_category` = cl.`id_cs_blog_category` AND cl.`id_cs_blog_category` != 1 AND cl.`id_lang` = '.(int)$id_lang.')';
		
		$sql .= ' LEFT JOIN `'._DB_PREFIX_.'cs_blog_category_shop` cs ON (c.`id_cs_blog_category` = cs.`id_cs_blog_category` AND cs.`id_shop` = '.(int)$id_shop.')';
		$sql .= ' WHERE c.`id_cs_blog_category` != 1';
		$sql .= ' AND c.`category_parent` = '.(int)$id_parent;
	...
	{
		$sql = 'SELECT id_cs_blog_category FROM '._DB_PREFIX_.'cs_blog_category_post
				WHERE id_cs_blog_post = '.$id_post;
		return (Db::getInstance()->ExecuteS($sql));
	}
}