...
class AdminCSPost extends ModuleAdminController
{
	protected $available_tabs_lang = array();
	
	protected $position_identifier = 'id_cs_blog_post';
	...
	function __construct()
	{
		...
		$this->className = 'CSBLPost';
		$this->table = 'cs_blog_post';
		...
		parent::__construct();
		//$this->_pagination = array(10, 20, 50, 100, 300);
		$this->multishop_context = -1;
		$this->multishop_context_group = true;
		$this->fieldImageSettings = array(
 			'name' => 'image',
 			'dir' => _PS_MODULE_DIR_.'csblog/media/posts/src'
 		);
		if (!Tools::getValue('id_cs_blog_post'))
	...
	public function init()
	{
		...
		if (Shop::getContext() == Shop::CONTEXT_SHOP)
			$this->_join .= ' LEFT JOIN `'._DB_PREFIX_.'cs_blog_post_shop` sa ON (a.`id_cs_blog_post` = sa.`id_cs_blog_post` AND sa.id_shop = '.(int)$this->context->shop->id.') ';
		else
			$this->_join .= ' LEFT JOIN `'._DB_PREFIX_.'cs_blog_post_shop` sa ON (a.`id_cs_blog_post` = sa.`id_cs_blog_post` AND sa.id_shop = 1) ';
	...
	public function postProcess()
	{
		...
		{
			$id = Tools::getValue('id_cs_blog_post');
			$link = self::$currentIndex.'&id_cs_blog_post='.$id.'&updatecs_blog_post&token='.$this->token;
	...
	public function displayImage($id_cs_blog_post)
	{
		$id_s = (int)Context::getContext()->shop->id;
		$id_shop = $id_s ? $id_s: Configuration::get('PS_SHOP_DEFAULT');
		if ($id_cs_blog_post) {
			$save_path = _PS_MODULE_DIR_.'csblog/media/posts/cache/'.$id_cs_blog_post.'_'.$id_shop.'_small';
			$url_path = '../modules/csblog/media/posts/cache/'.$id_cs_blog_post.'_'.$id_shop.'_small';
	...
	{
		if($id){
			$save_path = _PS_MODULE_DIR_.'csblog/media/posts/src/'.$id;
			$url_path_general = _PS_MODULE_DIR_.'csblog/media/posts/cache/'.$id;
		...
		foreach (Shop::getContextListShopID() as $id_shop)
		{
			...
			$this->imageResizeThumb($src_path,$des_path,Configuration::get('CS_IMG_SMALL_SIZE',null,null,$id_shop),Configuration::get('CS_IMG_SMALL_H_SIZE',null,null,$id_shop));
			
			# Medium image
			$des_path = $url_path_general.'_'.$id_shop.'_medium.'.$this->imageType;
			$this->imageResizeThumb($src_path,$des_path,Configuration::get('CS_IMG_MEDIUM_SIZE',null,null,$id_shop),Configuration::get('CS_IMG_MEDIUM_H_SIZE',null,null,$id_shop));
			
			# Large image
			$des_path = $url_path_general.'_'.$id_shop.'_large.'.$this->imageType;
			$this->imageResizeThumb($src_path,$des_path,Configuration::get('CS_IMG_LARGE_SIZE',null,null,$id_shop),Configuration::get('CS_IMG_LARGE_H_SIZE',null,null,$id_shop));
		}
									   ...
									   $use_shop_context = false)
	{
		...
		$this->context->controller->addJs(array(
			...
			__PS_BASE_URI__.'modules/csblog/js/admin-categories-tree.js'));
		if ($use_search)
	...
	public function renderForm()
	{
		...
		global $cookie;
		$this->addJS(array(
				__PS_BASE_URI__.'modules/csblog/js/csautopost.js'
		));
		$id_lang = $this->context->language->id;
		$id = (int)Context::getContext()->shop->id;
		$id_shop = $id ? $id: Configuration::get('PS_SHOP_DEFAULT');
		$html_image_des = '';
		$objCate = new CSPLCategory();
		$categories = $objCate->getCategories($id_lang,$id_shop,true);
		...
		$id_cs_blog_post = Tools::getValue('id_cs_blog_post');
		if($id_cs_blog_post)
		{
			$html_image_des .= $this->displayImage($id_cs_blog_post);
		}
		
		/*get data auto post + product*/
		$string_name = '';
		$array_id_product = explode('-',$this->object->id_related_products);
		$array_name_product = explode('¤',$this->object->name_related_products);
		foreach ($array_id_product as $k=>$id_product)
		{
			if($id_product !== end($array_id_product))
			{
				$string_name .= '<br />'.$array_name_product[$k].'
		...
		foreach ($array_id_post as $k=>$id_post)
		{
			...
			{
				$string_name_post .= '<br />'.$array_name_post[$k].'
				<span class="delPosts" name="'.$id_post.'" style="cursor: pointer;">
				<img src="../img/admin/delete.gif" alt="" />
				</span>';
			}
		}
		...
		{
			$categoriesSelected = CSPLCategory::getCategoriesSelected($this->object->id);
			...
			'input' => array(
				...
				array(
				 ...
				 array(
					...
					'delete_url' => self::$currentIndex.'&id_cs_blog_post='.$id_cs_blog_post.'&updatecs_blog_post&token='.$this->token.'&deleteimagePost',
				...
				array(
					...
					'id_related_products' => $this->object->id_related_products,
					'name_related_products' => $this->object->name_related_products,
	...
	{
		return mb_substr(strip_tags(stripslashes($description)),0,130);
	}
	
	public static function getNameCategory($id_category)
	{
		$cat_parrent = new CSPLCategory($id_category);
	...
	public function updateTags($languages, $post)
	{
		...
		if (!CSBLTag::deleteTagsForPost((int)$post->id))
		...
		{
			if ($value = Tools::getValue('id_cs_blog_tags_'.$language['id_lang']))
				$tag_success &= CSBLTag::addTags($language['id_lang'], (int)$post->id, $value);
		}
		if (!$tag_success)
