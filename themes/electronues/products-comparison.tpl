...
<h1 class="title">{l s='Product Comparison'}</h1>
...
<div style="width: {sizeof($products)*250+228}px" class="content_comapre">
	<table id="product_comparison">
		<tr>
			<td class="ttitle" style="width:280px">{l s="Product"}</td>	
			...
			<td style="width:270px" class="tproduct">				
		...
		<tr>
			<td style="width:280px" class="ttitle">{l s="Image"}</td>
			{foreach from=$products item=product name=for_products}
			{assign var='replace_id' value=$product->id|cat:'|'}
			<td style="width:270px" class="timage">
				<a class="cmp_remove" href="{$link->getPageLink('products-comparison', true)}" rel="ajax_id_product_{$product->id}" title="{l s='Remove'}">{l s='Remove'}</a>
				<div class="image">
		...
		<tr>
			<td style="width:280px" class="ttitle">{l s="Price"}</td>
			{foreach from=$products item=product name=for_products}
			{assign var='replace_id' value=$product->id|cat:'|'}
				<td style="width:270px" class="tprice">
		...
		<tr>
			<td style="width:280px" class="ttitle">
				{l s="Availibility"}
			</td>
			{foreach from=$products item=product name=for_products}
			{assign var='replace_id' value=$product->id|cat:'|'}
			<td style="width:270px" class="tavaili">
		...
		<tr>
			<td style="width:280px" class="ttitle"></td>
			{foreach from=$products item=product name=for_products}
			{assign var='replace_id' value=$product->id|cat:'|'}
			<td style="width:270px">
				{if (!$product->hasAttributes() OR (isset($add_prod_display) AND ($add_prod_display == 1))) AND $product->minimal_quantity == 1 AND $product->customizable != 2 AND !$PS_CATALOG_MODE}
					{if ($product->quantity > 0 OR $product->allow_oosp)}
						<a class="csbutton ajax_add_to_cart_button csdefault" rel="ajax_id_product_{$product->id}"  title="{l s='Add to cart'}"><span></span>{l s='Add to cart'}</a>
					{else}
						<span class="csbutton csdefault">{l s='Add to cart'}</span>
					{/if}
				{else}
