...
<h3 class="title1">{l s='Follow your order\'s status step-by-step'}</h3>
<br/>
...
{if !$is_guest}
	...
	<div id="returnOrderMessage">
		<h3 class="title1">{l s='Merchandise return'}</h3>
	...
	{if count($messages)}
	<h3 class="title1">{l s='Messages'}</h3>
	<div class="table_block">
		<table class="detail_step_by_step std">
			<thead>
				<tr>
	...
	<form action="{$link->getPageLink('order-detail', true)|escape:'html'}" method="post" class="std" id="sendOrderMessage">
		...
		<p class="submit">
			...
			<input type="submit" class="csbutton csdefault" name="submitMessage" value="{l s='Send'}"/>
		</p>
	</form>
{else}
