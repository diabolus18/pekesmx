...
<form action="{$request_uri|escape:'htmlall':'UTF-8'}" method="post" class="std" id="form_forgotpassword">
	<div class="fieldset">
		...
		<p class="required submit" style="margin-top:10px">
			<input type="submit" class="csbutton cssecond" value="{l s='Retrieve Password'}" />
		</p>
	</div>
</form>
{/if}
<p class="footer_links clear">
	<a href="{$link->getPageLink('authentication', true)}" title="{l s='Return to Login'}" rel="nofollow" class="csbutton csdefault"><img src="{$img_dir}icon/my-account.gif" alt="{l s='Return to Login'}"/>{l s='Back to Login'}</a>
</p>
